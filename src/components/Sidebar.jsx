...
export default function Sidebar() {
  const navItems = [
    { to: "/", icon: <LayoutDashboard size={20} />, activeIcon: <LayoutDashboard size={24} />, label: "Dashboard" },
    { to: "/nueva-venta", icon: <PlusCircle size={20} />, activeIcon: <PlusCircle size={24} />, label: "Nueva Venta" },
    { to: "/historial", icon: <History size={20} />, activeIcon: <History size={24} />, label: "Historial" },
  ];

  return (
    <>
      {/* Desktop Sidebar */}
      <aside className="hidden md:flex w-72 h-full flex-col bg-white/5 backdrop-blur-2xl border-r border-white/10 shadow-[4px_0_24px_rgba(0,0,0,0.4)] z-50">
      ...
      {/* Mobile Top Header */}
      <header className="md:hidden flex flex-col pt-6 pb-2 px-6 items-center justify-center relative z-40 bg-transparent mb-4">
        <div className="flex items-center gap-3">
          <div className="relative flex items-center justify-center bg-gradient-to-br from-white/10 to-white/5 p-2.5 rounded-xl border border-white/10 shadow-sm backdrop-blur-md">
             <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 24 24"
              fill="none"
              stroke="url(#rose-gradient-mobile)"
              strokeWidth="1.5"
              strokeLinecap="round"
              strokeLinejoin="round"
              className="w-7 h-7 drop-shadow-[0_0_8px_rgba(255,42,112,0.8)]"
            >
              <defs>
                <linearGradient id="rose-gradient-mobile" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#ffffff" />
                  <stop offset="50%" stopColor="#ff2a70" />
                  <stop offset="100%" stopColor="#ff0055" />
                </linearGradient>
          ...
          <div className="flex flex-col">
            <span className="font-extrabold text-2xl text-white tracking-tight drop-shadow-md leading-none">Angela</span>
            <span className="text-[10px] font-bold text-[#ff2a70] tracking-[0.25em] uppercase mt-1.5 drop-shadow-[0_0_5px_rgba(255,42,112,0.6)] leading-none text-left">Viajes</span>
          </div>
        </div>
      </header>
      ...
      <nav className="md:hidden fixed bottom-6 left-6 right-6 z-50">
        <div className="bg-black/60 backdrop-blur-3xl border border-white/10 shadow-[0_15px_40px_rgba(0,0,0,0.6)] rounded-2xl flex items-center justify-around py-3 px-2">
          {navItems.map((item) => (
            <NavLink
              key={item.to}
              to={item.to}
              className={({ isActive }) =>
                `flex flex-col items-center gap-1.5 p-2 rounded-xl transition-all duration-300 relative w-20 ${isActive
                  ? "text-white"
            ...
            >
              {({ isActive }) => (
                <>
                  <div className={`transition-all duration-300 flex items-center justify-center ${isActive ? 'text-[#ff2a70] drop-shadow-[0_0_10px_rgba(255,42,112,0.9)] -translate-y-[2px]' : ''}`}>
                    {isActive ? item.activeIcon : item.icon}
                  </div>
                  <span className={`text-[10px] font-bold transition-all duration-300 tracking-wide text-center uppercase ${isActive ? 'opacity-100 translate-y-0 text-white' : 'opacity-0 translate-y-2 absolute bottom-1 pointer-events-none'}`}>
                    {item.label}
                  </span>
                  {isActive && (
                    <div className="absolute -bottom-1.5 left-1/2 -translate-x-1/2 w-1 h-1 bg-[#ff2a70] rounded-full shadow-[0_0_8px_2px_rgba(255,42,112,0.8)]" />
                  )}
                </>
              )}
            </NavLink>
